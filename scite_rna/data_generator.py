...
class DataGenerator: 
    ...
    def generate_reads(self, new_tree=False, new_mut_type=False, new_coverage=True):
        if new_mut_type:
        ...
        # determine genotypes
        genotype = np.empty((self.n_cells, self.n_mut), dtype=str)
        ...
        for i in range(self.n_cells):
            for j in range(self.n_mut):
                ref[i,j], alt[i,j] = self.generate_single_read(genotype[i,j], self.coverage[i,j])

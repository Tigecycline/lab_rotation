...
class TreeOptimizer: 
    ...
    def fit(self, ref, alt, gt1, gt2, reversible = None): 
        assert(ref.shape == alt.shape)
        self.n_cells, self.n_mut = ref.shape
        assert(len(gt1) == len(gt2) == self.n_mut)
        
        self.likelihoods1, self.likelihoods2 = cell_locus_likelihoods(ref, alt, gt1, gt2)
        # TBC: use a boolean numpy array instead
        ...
        self.ct_LLR = np.empty((self.ct.n_nodes, self.n_mut)) # [i,j] log-likelihood ratio of mutation j attached to edge above cell i in the cell tree
        ...
        if reversible is None or reversible == False: 
            self.reversible = []
    ...
    def ct_joint(self): 
        ...
        # sum seems faster than numpy.sum when summing up lists
        # but the opposite is true for numpy arrays
        return sum([self.ct_L1[j] + self.ct_LLR[self.ct.attachments[j], j] for j in range(self.n_mut)])
    
    
    @property
    def mt_joint(self): 
        ''' joint likelihood of the mutation tree with attached cells '''
        return sum([self.mt_L[i, self.mt.attachments[i]] for i in range(self.n_cells)])
    ...
    def ct_hill_climb(self, convergence = 64, timeout = np.inf, weights = [0.5, 0.5]): 
    ...
    def mt_hill_climb(self, convergence = 128, timeout = np.inf, weights = [0.5, 0.5]): 
        n_proposed = 0 # number of failed moves
        n_steps = 0
    ...
    def optimize(self, start_space = 0, strategy = 'hill climb'): 
        ...
        while not all(converged): 
            ...
            if current_space == 0: 
                new_history = self.ct_hill_climb(convergence = ct_convergence, timeout = ct_timeout)
                self.mt.fit_structure(self.ct)
                self.mt_L[:,self.mt.root.ID] = np.sum(self.likelihoods1, axis = 1)
                self.update_mt()

            elif current_space == 1: 
                new_history = self.mt_hill_climb(convergence = mt_convergence, timeout = mt_timeout)
